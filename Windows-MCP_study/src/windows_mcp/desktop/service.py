...
class Desktop:
    ...
    def get_state(
        self, use_vision: bool = False, as_bytes: bool = False, scale: float = 1.0
    ) -> DesktopState:
        sleep(0.1)
        apps = self.get_apps()
        ...
        if use_vision:
            screenshot = self.tree.get_annotated_screenshot(
                tree_state.interactive_nodes, scale=scale
            )
            if as_bytes:
                bytes_io = io.BytesIO()
    ...
    def get_window_element_from_element(
        self, element: uia.Control
    ) -> uia.Control | None:
    ...
    def switch_app(self, name: str):
        apps = {
            app.name: app
            for app in [self.desktop_state.active_app] + self.desktop_state.apps
            if app is not None
        }
