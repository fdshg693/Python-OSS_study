...
async def lifespan(app: FastMCP):
    ...
    try:
        log.info(
            "server_starting",
            server_name="windows-mcp-custom",
            windows_version=windows_version,
        )
        watch_cursor.start()
        await asyncio.sleep(1)  # Simulate startup latency
        log.info("server_started", server_name="windows-mcp-custom")
        yield
    finally:
        log.info("server_stopping", server_name="windows-mcp-custom")
        watch_cursor.stop()
        log.info("server_stopped", server_name="windows-mcp-custom")


mcp = FastMCP(name="windows-mcp-custom", instructions=instructions, lifespan=lifespan)
...
@log_tool_call
def type_tool(
    loc: list[int], text: str, clear: bool = False, press_enter: bool = False
) -> str:
    if len(loc) != 2:
        raise ValueError("Location must be a list of exactly 2 integers [x, y]")
    x, y = loc[0], loc[1]
